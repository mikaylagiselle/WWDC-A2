...
# Timer variables
timer_duration = 15  # Duration of the timer in seconds
...
def draw():
    ...
    if mood == "":
        ...
        if spinning:
            rotation_angle += 10  # Increment rotation angle
...
def keyPressed():
    ...
    # Allow input of characters
    elif keyCode == BACKSPACE:
        mood = mood[:-1]  # Remove last character
...
def drawLines(x):
    ...
    for i in range(0, width, 30):  # Vertical spacing of lines
        y_offset = 50 * sin(radians(i + x))  # Dynamic wave based on time
        line(i, height / 2 + y_offset, i, height / 2 + height * 0.2)  # Draw lines
...
def displayMood(mood):
    if mood.lower() == "happy":
        ...
        fill(0)
        text("Thanks for contributing! Every mood is important!", width / 2, height / 2 + 160)
...
def drawHappyArt():
    ...
    text("That's great to hear! Click the circles for a happy surprise! :)", width / 2, height / 2 + 160)
...
def drawSadArt():
    ...
    text("Cheer up by clicking this silly sun! Hope you feel better.", sun_x, sun_y + 100)
...
def drawCalmingArt():
    ...
    small_circle_size = circle_size * 0.5  # Smaller circle size

    # Center the main circle on the canvas 
    fill(173, 216, 230) # Steel blue for the main circle
    ellipse(width / 2, height / 2 + 50, circle_size, circle_size)
    ...
    for i in range(3):  # Draw 3 smaller circles
        ...
        ellipse(width / 2, height / 2 + 50, small_circle_size + offsets[i], small_circle_size + offsets[i])
    ...
    text("Let's do some calming breathing exercises! Breathe in and out with the circle.", width / 2, height / 2 + 160)
